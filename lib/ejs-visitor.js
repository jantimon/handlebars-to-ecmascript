...
function staticMemberExpression (members) {
  function createExpression (parts) {
    ...
    return {
      ...
      property: { type: 'Literal', value: identifier }
    };
  }
  return createExpression(members.split('.'));
}

function createImport (name, pkg) {
  return {
...
class EcmaScriptVisitor extends Visitor {
  ...
  constructor (options) {
    ...
    this._options.importEscape = this._options.importEscape || this._options.export;
  ...
  Program (program) {
    ...
    if (this._options.importEscape && this._usedUtils.escape) {
      jsProgram.body.unshift(createImport('escape', 'lodash.escape'));
    }
  ...
  BlockStatement (block) {
    const helperName = kebabCase.reverse(block.path.parts[0]);
